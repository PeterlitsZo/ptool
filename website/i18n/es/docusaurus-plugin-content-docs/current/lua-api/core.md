...
> `v0.1.0` - Introducido. `v0.7.0` - Se añadieron expresiones de requisito de versión al estilo Cargo.

`ptool.use` declara la versión o el requisito de versión de `ptool` que necesita un script.
...
- El argumento acepta una cadena de versión semántica simple o una expresión de requisito de versión al estilo Cargo.
- Una cadena de versión simple admite un prefijo `v` opcional, como `v0.1.0` o `0.1.0`, y mantiene el comportamiento histórico de declarar la versión mínima requerida de `ptool`.
- Las expresiones de requisito admiten operadores y patrones como `^0.6.0`, `~0.6.0`, `>=0.6.0, <0.7.0`, `1.*` y `1.2.*`.
...
- `run` (table, opcional): Configuración por defecto de `ptool.run`, `ptool.run_capture`, `ptool.pipe` y `ptool.run_shell`. Campos admitidos:
...
- `ptool.try` es la forma recomendada de manejar errores de APIs como `ptool.fs.read`, `ptool.http.request`, `ptool.run(..., { check = true })`, `ptool.pipe(..., { check = true })` y `res:assert_ok()`.
...
- `stdin` (string|table, opcional): Fuente de stdin del proceso hijo.
  ...
  - Si se omite, el proceso hijo hereda el stdin del proceso actual.
...
- `stdout` (string|table, opcional): Estrategia de manejo de stdout.
  - `"inherit"`: hereda del terminal actual (predeterminado).
  - `"capture"`: Capturar en `res.stdout`.
  - `"null"`: Descarta la salida.
  - `{ file = "path" }`: Escribe stdout en un archivo, truncándolo primero.
  - `{ file = "path", append = true }`: Añade stdout al final de un archivo.
- `stderr` (string|table, opcional): Estrategia de manejo de stderr.
  - `"inherit"`: hereda del terminal actual (predeterminado).
...
`ptool.pipe(stages[, options])` ejecuta un pipeline local sin invocar un shell.

- `stages` (table, obligatorio): Lista de etapas del pipeline. Se requieren al menos dos etapas.
- `options` (table, opcional): Sobrescrituras por invocación.
- Devuelve: Una tabla de resultado similar a `ptool.run(...)`.

Reglas de las etapas:

- Cada etapa puede ser una cadena de comando como `"printf hello"`.
- Cada etapa puede ser un arreglo como `{"tr", "a-z", "A-Z"}`, donde el primer elemento es el comando y el resto son argumentos.
- Cada etapa puede ser una tabla como `{ cmd = "tr", args = {"a-z", "A-Z"} }`.
- Las cadenas de comando de las etapas y las cadenas de argumentos se dividen usando reglas de estilo shell (`shlex`), igual que en `ptool.run(...)`.

Reglas del valor devuelto:

- `ok` (boolean): Si todas las etapas salieron con estado `0`.
- `code` (integer|nil): Estado resumido del pipeline. Si tiene éxito, este es el estado de la última etapa. Si falla, es el último estado distinto de cero, o `nil` si la etapa relevante terminó por una señal.
- `codes` (integer|nil)[]: Estados de salida por etapa en el orden de las etapas.
- `cmd` (string): Forma visible del pipeline completo, como ``printf hello | tr a-z A-Z``.
- `cwd` (string): Directorio de trabajo efectivo usado para el pipeline.
- `stdout` (string, opcional): Presente cuando `stdout = "capture"`. Captura la salida estándar de la última etapa.
- `stderr` (string, opcional): Presente cuando `stderr = "capture"`. Combina la salida de error de todas las etapas en el orden de las etapas.
- `assert_ok(self)` (function): Lanza la misma forma de error estructurado `command_failed` que `ptool.run(...)` cuando falla cualquier etapa.

Comportamiento:

- `options` admite los mismos campos por invocación que `ptool.run(options)`, salvo que `cmd` y `args` se sustituyen por la tabla posicional `stages`.
- Los valores por defecto de `echo`, `check`, `confirm` y `retry` siguen viniendo de `ptool.config({ run = { ... } })`.
- `stdin` se conecta a la primera etapa.
- `stdout` se refiere a la salida de la última etapa.
- `stderr` se aplica a todas las etapas.
- Cuando `check = true`, cualquier etapa fallida produce un error estructurado `command_failed`, y `retry = true` pregunta si debe volver a ejecutarse todo el pipeline.
...
Admite las mismas formas de llamada de argumentos de comando que `ptool.run`:
...
- Si tiene éxito, `ptool.exec` no devuelve. El proceso actual de `ptool` es reemplazado por el comando de destino.
- No se ejecuta ningún código Lua después de una llamada exitosa a `ptool.exec(...)`.
- En `ptool repl`, una llamada exitosa a `ptool.exec(...)` también reemplaza el proceso del REPL.
- En plataformas Unix, esto usa semántica real de reemplazo de proceso.
- En plataformas no Unix, `ptool.exec` actualmente produce un error `unsupported`.

Reglas de argumentos:

- `ptool.exec(cmdline)`: `cmdline` se divide usando reglas de estilo shell (`shlex`). El primer elemento se trata como comando y el resto como argumentos.
- `ptool.exec(cmd, argsline)`: `cmd` se usa directamente como comando y `argsline` se divide en una lista de argumentos con reglas de estilo shell (`shlex`).
- `ptool.exec(cmd, args)`: `cmd` es una cadena y `args` es un arreglo de cadenas.
- `ptool.exec(cmdline, options)`: `options` sobrescribe la configuración de esta invocación, por ejemplo `echo`.
- `ptool.exec(cmd, args, options)`: `args` puede ser una cadena o un arreglo de cadenas, y `options` sobrescribe la configuración de esta invocación, por ejemplo `confirm`.
- `ptool.exec(options)`: `options` es una tabla.
- Cuando el segundo argumento es una tabla: si es un arreglo (claves enteras consecutivas `1..n`), se trata como `args`; en caso contrario se trata como `options`.

También se admite `ptool.exec(options)`, donde `options` es una tabla con los siguientes campos:
...
- `confirm` (boolean, opcional): Si debe pedirse confirmación al usuario antes del reemplazo. Si se omite, se usa el valor de `ptool.config({ run = { confirm = ... } })`; si tampoco está definido, el valor predeterminado es `false`.
- `stdout` (string|table, opcional): Estrategia de manejo de stdout.
  - `"inherit"`: hereda del terminal actual (predeterminado).
  - `"null"`: Descarta la salida.
  - `{ file = "path" }`: Escribe stdout en un archivo, truncándolo primero.
