...
- Retorna: O valor Lua analisado. A raiz pode ser qualquer tipo YAML
  compatível.
...
- Também ocorre erro se o valor YAML não puder ser representado como um valor
  Lua de `ptool`, como um mapping com chaves não string ou um valor com tag
  YAML explícita.
...
- `path` ((string|integer)[], obrigatório): Um array de caminho não vazio, como
  `{"spec", "template", "metadata", "name"}` ou `{"items", 1, "name"}`.
...
- Segmentos de caminho integer selecionam elementos de sequências usando índices
  Lua base 1.
...
- `value` (valor Lua compatível com YAML, obrigatório): O valor a ser
  codificado.
...
- O argumento `path` de `ptool.yaml.get` deve ser um array não vazio de strings
  e/ou inteiros positivos.
