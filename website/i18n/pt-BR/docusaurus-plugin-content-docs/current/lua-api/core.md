...
> `v0.1.0` - Introduced. `v0.7.0` - Added Cargo-style version requirement expressions.

`ptool.use` declara a versão ou o requisito de versão de `ptool` necessário para um script.
...
- O argumento aceita tanto uma string simples de versão semântica quanto uma expressão de requisito de versão no estilo Cargo.
- Uma string simples de versão aceita um prefixo `v` opcional, como `v0.1.0` ou `0.1.0`, e mantém o comportamento histórico de declarar a versão mínima necessária de `ptool`.
- As expressões de requisito aceitam operadores e padrões como `^0.6.0`, `~0.6.0`, `>=0.6.0, <0.7.0`, `1.*` e `1.2.*`.
...
`ptool.is_root()` retorna se o processo atual do `ptool` está sendo executado como o usuário root.

- Retorna: `boolean`.

Comportamento:

- Em plataformas do tipo Unix, isso verifica se o usuário efetivo atual é root.
- Em plataformas não Unix, atualmente isso retorna `false`.
- Isso é útil junto com `sudo = true` em helpers de comando locais como `ptool.run(...)` e `ptool.exec(...)`.
...
`ptool.which(command)` verifica se um comando local está disponível para execução.

- `command` (string, obrigatório): O nome do comando ou caminho do executável a verificar.
- Retorna: `string|nil`.
- Quando o comando é encontrado, o valor retornado é o caminho resolvido do executável.
- Quando o comando não é encontrado, o valor retornado é `nil`.
- Nomes de comando vazios geram um erro.

Comportamento:

- Nomes de comando sem separadores de caminho são resolvidos pelo `PATH` efetivo atual, incluindo alterações feitas com `ptool.os.setenv(...)` e `ptool.os.unsetenv(...)`.
- Comandos com formato de caminho são verificados diretamente. Caminhos relativos são resolvidos a partir do diretório de trabalho atual do `ptool`.
- Em plataformas tipo Unix, o destino deve ser um arquivo com permissão de execução.
- No Windows, a busca também respeita `PATHEXT` para extensões executáveis.
...
`ptool.which_or_fatal(command)` verifica se um comando local está disponível e gera um erro se ele estiver ausente.

- `command` (string, obrigatório): O nome do comando ou caminho do executável a verificar.
- Retorna: `string`.
- Quando o comando é encontrado, o valor retornado é o caminho resolvido do executável.
- Quando o comando não é encontrado, o script sai com um erro.
- Nomes de comando vazios geram um erro.

Comportamento:

- A busca segue as mesmas regras de `ptool.which(...)`.
- Use isso quando um script exigir uma ferramenta externa antes de continuar.
...
- `sudo` (boolean, optional): Se deve executar o comando via `sudo` quando o usuário atual não for root. O padrão é `false`.
...
- Quando formas de chamada abreviadas como `ptool.run(cmdline, options)` ou `ptool.run(cmd, args, options)` são usadas, a tabela `options` por chamada também aceita `sudo`, `stdin` e `trim` com o mesmo significado.
...
- Quando `sudo = true`:
  - Se o usuário atual já for root, `ptool.run(...)` executa o comando diretamente sem adicionar `sudo`.
  - Caso contrário, `ptool.run(...)` executa o comando como `sudo <cmd> <args...>`.
  - No Windows, atualmente isso gera um erro `unsupported`.
- Quando `confirm = true`:
...
Ele suporta as mesmas formas de chamada de argumentos de comando que `ptool.run`:
...
- Em caso de sucesso, `ptool.exec` não retorna. O processo atual do `ptool` é substituído pelo comando de destino.
- Nenhum código Lua após uma chamada bem-sucedida de `ptool.exec(...)` é executado.
- No `ptool repl`, uma chamada bem-sucedida de `ptool.exec(...)` também substitui o próprio processo do REPL.
- Em plataformas Unix, isso usa semântica real de substituição de processo.
- Em plataformas não Unix, `ptool.exec` atualmente gera um erro `unsupported`.

Regras de argumento:

- `ptool.exec(cmdline)`: `cmdline` é dividido usando regras no estilo shell (`shlex`). O primeiro item é tratado como comando e o restante como argumentos.
- `ptool.exec(cmd, argsline)`: `cmd` é usado diretamente como comando e `argsline` é dividido em uma lista de argumentos usando regras no estilo shell (`shlex`).
- `ptool.exec(cmd, args)`: `cmd` é uma string e `args` é um array de strings.
- `ptool.exec(cmdline, options)`: `options` sobrescreve configurações desta invocação, como `echo`.
- `ptool.exec(cmd, args, options)`: `args` pode ser uma string ou um array de strings, e `options` sobrescreve configurações desta invocação, como `confirm`.
- `ptool.exec(options)`: `options` é uma tabela.
- Quando o segundo argumento é uma tabela: se for um array (chaves inteiras consecutivas `1..n`), é tratado como `args`; caso contrário, é tratado como `options`.

`ptool.exec(options)` também é suportado, em que `options` é uma tabela com os seguintes campos:

- `cmd` (string, obrigatório): O nome do comando ou caminho do executável.
- `args` (string[], opcional): A lista de argumentos.
- `cwd` (string, opcional): O diretório de trabalho do processo filho.
- `env` (table, opcional): Variáveis de ambiente adicionais, em que chaves são nomes de variáveis e valores são valores de variáveis.
- `sudo` (boolean, optional): Se deve executar o comando de substituição via `sudo` quando o usuário atual não for root. O padrão é `false`.
...
- Os caminhos de redirecionamento para arquivos são resolvidos em relação ao `cwd` efetivo do processo filho, a menos que um caminho absoluto seja fornecido.
- Quando `sudo = true`:
  - Se o usuário atual já for root, `ptool.exec(...)` substitui o processo atual diretamente pelo comando.
  - Caso contrário, `ptool.exec(...)` substitui o processo atual por `sudo <cmd> <args...>`.
  - No Windows, atualmente isso gera um erro `unsupported`.
- Quando `confirm = true`:
  - Se o usuário recusar a execução, um erro é gerado imediatamente.
  - Se o ambiente atual não for interativo (sem TTY), um erro é gerado imediatamente.
