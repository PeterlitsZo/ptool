...
- `version`（string | nil）：对象版本（如果可用）。
- `metadata`（table | nil）：用户定义的对象元信息（如果可用）。
...
- `path`（string，必填）：对象路径。
- `options`（table，选填）：读取选项。
- 返回：`string`。

`options` 字段：

- `range`（table，选填）：使用半开区间 `[start, end)` 读取字节范围。
  - `start`（integer，选填）：要包含的首个字节偏移。
  - `end`（integer，选填）：要排除的首个字节偏移。

行为：

- 省略 `range` 时读取整个对象。
- `{ start = N }` 从字节 `N` 读取到结尾。
- `{ end = N }` 从开头读取到字节 `N` 之前，但不包含字节 `N`。
- `{ start = A, end = B }` 读取从字节 `A` 到 `B - 1` 的内容。
...
- `options`（table，选填）：写入选项。
- 返回：`table`。

`options` 字段：

- `content_type`（string，选填）：设置对象内容类型。
- `cache_control`（string，选填）：设置对象 cache-control 头。
- `content_disposition`（string，选填）：设置对象 content-disposition 头。
- `content_encoding`（string，选填）：设置对象 content-encoding 头。
- `metadata`（table，选填）：将用户定义元信息设置为字符串键值对。
- `if_not_exists`（boolean，选填）：仅当对象尚不存在时写入。默认为 `false`。
- `if_match`（string，选填）：仅当当前 ETag 匹配时写入。
- `if_none_match`（string，选填）：仅当当前 ETag 不匹配时写入。

行为：

- `content` 会按字节原样上传。
- 嵌入的 NUL 字节和非 UTF-8 字节会被保留。
- 返回的条目可避免为了常见元信息立刻再调用一次 `stat()`。
- 某些 S3 兼容服务可能不会在写入响应中回显用户定义的 `metadata` 或 `etag`。发生这种情况时，这些字段会保持为 `nil`，直到后续调用 `stat()`。
...
规范 API 名称：`ptool.s3.Connection:put_bucket_acl`。

`conn:put_bucket_acl(options)` 会替换 bucket ACL，成功时返回 `nil`。

- `options`（table，必填）：bucket ACL 选项。
- 返回：`nil`。

`options` 字段：

- `acl`（string，选填）：预定义 bucket ACL。可选值为 `"authenticated-read"`、`"private"`、`"public-read"` 和 `"public-read-write"`。
- `expected_bucket_owner`（string，选填）：如果 bucket 不属于此 AWS 账户 ID，则请求失败。
- `grant_full_control`（string，选填）：`x-amz-grant-full-control` 请求头的值。
- `grant_read`（string，选填）：`x-amz-grant-read` 请求头的值。
- `grant_read_acp`（string，选填）：`x-amz-grant-read-acp` 请求头的值。
- `grant_write`（string，选填）：`x-amz-grant-write` 请求头的值。
- `grant_write_acp`（string，选填）：`x-amz-grant-write-acp` 请求头的值。

行为：

- 请提供 `acl`，或者一个或多个 `grant_*` 字段。这两种形式不能在一次调用中同时使用。
- 所有已提供的字符串都必须非空。
- 授权字符串使用 S3 授权请求头语法，例如目标服务提供商接受的账户 ID、电子邮件地址或组 URI。
- 该操作以连接的 bucket 为目标，不会应用连接的 `root` 前缀。
- 许多 S3 兼容服务提供商会禁用 ACL，或未实现 ACL API。在这种情况下，服务提供商错误会以 `s3_error` 返回。
...
规范 API 名称：`ptool.s3.Connection:put_object_acl`。

`conn:put_object_acl(path, options)` 会替换对象的 ACL，成功时返回 `nil`。

- `path`（string，必填）：对象路径。
- `options`（table，必填）：对象 ACL 选项。
- 返回：`nil`。

`options` 字段：

- `acl`（string，选填）：预定义对象 ACL。可选值为 `"authenticated-read"`、`"aws-exec-read"`、`"bucket-owner-full-control"`、`"bucket-owner-read"`、`"private"`、`"public-read"` 和 `"public-read-write"`。
- `expected_bucket_owner`（string，选填）：如果 bucket 不属于此 AWS 账户 ID，则请求失败。
- `grant_full_control`（string，选填）：`x-amz-grant-full-control` 请求头的值。
- `grant_read`（string，选填）：`x-amz-grant-read` 请求头的值。
- `grant_read_acp`（string，选填）：`x-amz-grant-read-acp` 请求头的值。
- `grant_write`（string，选填）：`x-amz-grant-write` 请求头的值。
- `grant_write_acp`（string，选填）：`x-amz-grant-write-acp` 请求头的值。
- `version_id`（string，选填）：将 ACL 应用于此对象版本。
- `request_payer`（string，选填）：仅接受 `"requester"`，并发送请求者付费确认。

行为：

- 请提供 `acl`，或者一个或多个 `grant_*` 字段。这两种形式不能在一次调用中同时使用。
- 所有已提供的字符串都必须非空。
- 授权字符串使用目标服务提供商接受的 S3 授权请求头语法。
- 前导 `/` 会被忽略，并且连接的 `root` 前缀会像其他对象操作一样应用到对象 key。
- 许多 S3 兼容服务提供商会禁用 ACL，或未实现 ACL API。在这种情况下，服务提供商错误会以 `s3_error` 返回。
