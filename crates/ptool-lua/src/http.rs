...
pub(crate) fn request(
    ...
    let options = parse_request_options(lua, options, REQUEST_SIGNATURE)?;
    let response = engine.http_request(options).map_err(to_lua_request_error)?;
    Ok(HttpResponse::from_engine(response))
}
...
pub(crate) fn parse_request_options(
    ...
    validate_request_option_keys(&options, signature)?;
    ...
    let headers = parse_headers(options.get::<Option<Table>>("headers")?, signature)?;
    let body = parse_body(options.get::<Option<Value>>("body")?, signature)?;
    let query = parse_string_pairs(options.get::<Option<Table>>("query")?, "query", signature)?;
    let json = parse_json_body(lua, options.get::<Option<Value>>("json")?, signature)?;
    let form = parse_string_pairs(options.get::<Option<Table>>("form")?, "form", signature)?;
    ...
    let basic_auth = parse_basic_auth(options.get::<Option<Table>>("basic_auth")?, signature)?;
...
fn validate_request_option_keys(options: &Table, signature: &str) -> mlua::Result<()> {
...
fn parse_headers(headers: Option<Table>, signature: &str) -> mlua::Result<Vec<(String, String)>> {
    ...
    for pair in headers.pairs::<Value, Value>() {
        let (name, value) = pair?;
        let name = parse_string_key(name, "headers", signature)?;
        let value = parse_string_value(value, "headers", signature)?;
...
fn parse_string_pairs(
    ...
    for pair in table.pairs::<Value, Value>() {
        let (key, value) = pair?;
        let key = parse_string_key(key, field, signature)?;
        let value = parse_string_like_value(value, field, signature)?;
...
fn parse_body(body: Option<Value>, signature: &str) -> mlua::Result<Option<Vec<u8>>> {
...
fn parse_json_body(
    ...
    crate::json::lua_value_to_json(lua, value, &format!("{signature} `json` has invalid value"))
        .map(Some)
}
...
fn parse_basic_auth(
    ...
    for pair in value.pairs::<Value, Value>() {
        let (key, _) = pair?;
        let key = parse_string_key(key, "basic_auth", signature)?;
        match key.as_str() {
            "username" | "password" => {}
            _ => {
                return Err(crate::lua_error::invalid_option(
...
fn parse_string_key(value: Value, field: &str, signature: &str) -> mlua::Result<String> {
    match value {
...
fn parse_string_value(value: Value, field: &str, signature: &str) -> mlua::Result<String> {
    match value {
        Value::String(value) => Ok(value.to_str()?.to_string()),
        _ => Err(crate::lua_error::invalid_argument(
            signature,
...
fn parse_string_like_value(value: Value, field: &str, signature: &str) -> mlua::Result<String> {
    match value {
        Value::String(value) => Ok(value.to_str()?.to_string()),
...
impl HttpResponse {
    pub(crate) fn from_engine(inner: EngineHttpResponse) -> Self {
        Self { inner }
    }
