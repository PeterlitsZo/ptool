...
fn create_ptool_module(
    ...
    let version_state = Rc::clone(&world);
    let version_fn = lua.create_function(move |_, ()| Ok(version_state.borrow().version()))?;
    let is_root_state = Rc::clone(&world);
    let is_root_fn = lua.create_function(move |_, ()| Ok(is_root_state.borrow().is_root()))?;
...
fn create_ptool_consul_module(
    lua: &Lua,
    world: Rc<RefCell<crate::LuaWorld>>,
) -> mlua::Result<Table> {
    let consul_module = lua.create_table()?;
    let connect_fn =
...
fn create_ptool_fs_module(lua: &Lua, world: Rc<RefCell<crate::LuaWorld>>) -> mlua::Result<Table> {
    ...
    let open_state = Rc::clone(&world);
    let open_fn = lua.create_function(move |_, (path, mode): (String, Option<String>)| {
        open_state.borrow().fs_open(path, mode)
    })?;
