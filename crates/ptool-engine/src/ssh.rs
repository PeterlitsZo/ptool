...
fn run_ssh_command(
    ...
    let stdout_handle = child
        .stdout
        .take()
        .map(|reader| spawn_process_output_thread(reader, &stdout_mode, false, console));
    let stderr_handle = child
        .stderr
        .take()
        .map(|reader| spawn_process_output_thread(reader, &stderr_mode, true, console));
    ...
    R: Read + Send + 'static,
{
    match mode {
        ...
        ProcessOutputMode::Inherit => {
            ProcessOutputHandle::Inherit(spawn_prefixed_output_thread(reader, use_stderr, console))
        }
